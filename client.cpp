...
bool generate_mlkem(OQS_KEM*& kem, std::vector<uint8_t>& pk, std::vector<uint8_t>& sk) {
    try {
        ...
        if (!kem) throw std::runtime_error("[!] Failed to initialize ML-KEM algorithm");
...
bool derive_hybrid_key(const std::vector<uint8_t>& shared_mlkem,
    ...
    try {
        ...
        if (!ok) throw std::runtime_error("[!] HKDF derivation failed");
...
int main() {
    try {
        ...
        if (!post_key_exchange("http://localhost:5000/key_exchange", request, response)) return 1;
        ...
        std::vector<uint8_t> hybrid_key;
        if (!derive_hybrid_key(shared_mlkem, shared_ecdh, hybrid_key)) return 1;

        // Decrypt the message from the server using the hybrid key.
        std::vector<uint8_t> plaintext;
        if (!aes_gcm_decrypt(hybrid_key, iv, ciphertext, tag, plaintext)) return 1;
