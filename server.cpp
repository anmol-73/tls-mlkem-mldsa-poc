...
#include "json.hpp"               // JSON for Modern C++ (used for HTTP message parsing)
...
void key_exchange_handler(const httplib::Request &req, httplib::Response &res,
                          EVP_PKEY *server_ecdh_priv, const std::vector<uint8_t>& server_ecdh_pub_pem) {
    std::cout << "-------------------------------\n";
    std::cout << "[+] Received /key_exchange request\n";
    try {
        // Parse client JSON and decode the Base64-encoded ML-KEM and ECDHE public keys.
...
int main() {
    try {
        ...
        svr.Post("/key_exchange", [&](const httplib::Request& req, httplib::Response& res) {
            key_exchange_handler(req, res, server_ecdh_priv, server_ecdh_pub_pem);
        });
